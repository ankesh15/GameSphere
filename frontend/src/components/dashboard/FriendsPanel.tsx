...
import { Wifi, WifiOff, Clock, Gamepad2 } from "lucide-react";
...
const statusConfig = {
  ...
  "in-game": { dot: "bg-brand-500", label: "In Game", textColor: "text-brand-400" },
  idle: { dot: "bg-amber-500", label: "Idle", textColor: "text-amber-400" },
        ...
        {friends.map((friend, index) => {
          ...
          return (
            <motion.div
              ...
              className="group flex items-center gap-3 rounded-xl px-3 py-2 transition-colors hover:bg-slate-800/30 cursor-pointer"
            >
              {/* Avatar */}
              <div className="relative">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-800 border border-slate-700/30 text-[10px] font-bold text-slate-400">
                  {initials}
                </div>
                <span className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-slate-900 ${s.dot}`} />
              </div>
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className={`text-[10px] font-medium ${s.textColor} flex items-center gap-1`}>
